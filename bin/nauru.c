...
void init_nauru() {
    ...
    for (int r = 0; r < SIZE; r++) {
        for (int c = 0; c < SIZE; c++) {
            // Dans ce nouveau repère, r=0 est le HAUT (Ligne I) et r=8 est le BAS (Ligne A)
...
void print_nauru_board() {
    ...
    printf("\n  +-------------------+\n");
    for (int r = 0; r < SIZE; r++) {
        // L'affichage va de haut en bas. 
        // Si r=0, 'I' - 0 = 'I' (Haut). Si r=8, 'I' - 8 = 'A' (Bas).
        printf("%c | ", 'I' - r); 
        for (int c = 0; c < SIZE; c++) {
    ...
    printf("  +-------------------+\n");
    // Les colonnes sont bien les chiffres en bas
...
int check_move(int sr, int sc, int er, int ec, int player, int mode) {
    ...
    if (mode == 1) { 
        ...
        if (grid[er - step_r][ec - step_c] != opp) return 0; 
...
void play_nauru() {
    // Variables locales avec taille RÉDUITE à 16 octets.
    // Cela empêche le Stack Overflow ET esquive le bug du .bss.
    char cmd[16]; 
    char src[16]; 
    char dst[16];
    ...
    while(1) {
        ...
        // --- SAISIE DES COORDONNEES SEPARÉES ---
        ...
        if (strlen(src) < 2 || strlen(dst) < 2) {
            printf("/!\\ Format invalide. Utilisez une Lettre et un Chiffre (ex: A1).\n");
            continue;
        }

        // --- NOUVEAU MAPPAGE : Lettre = Ligne (A à I), Chiffre = Colonne (1 à 9) ---
        // r = 0 correspond au Haut ('I'). r = 8 correspond au Bas ('A').
        int sr;
        if (src[0] >= 'a' && src[0] <= 'i') sr = 'i' - src[0];
        else if (src[0] >= 'A' && src[0] <= 'I') sr = 'I' - src[0];
        else sr = -1; // Lettre invalide

        int sc = src[1] - '1';                                  

        int er;
        if (dst[0] >= 'a' && dst[0] <= 'i') er = 'i' - dst[0];
        else if (dst[0] >= 'A' && dst[0] <= 'I') er = 'I' - dst[0];
        else er = -1; // Lettre invalide
        ...
        if (res == 2) {
            while(1) {
                ...
                // On recalcule la lettre associée à la coordonnée 'er'
                printf("Nouvelle case d'arrivee depuis %c%d : ", 'I' - er, ec + 1);
                memset(dst, 0, 16);
                scanf("%s", dst);
                ...
                int n_er;
                if (dst[0] >= 'a' && dst[0] <= 'i') n_er = 'i' - dst[0];
                else if (dst[0] >= 'A' && dst[0] <= 'I') n_er = 'I' - dst[0];
                else n_er = -1;
